...
namespace Items
{
    public class Item : MonoBehaviour
    {
        ...
        public void Initialize(ItemColors.IColor color, int order, float speed, GameManager gameManager,
     float initialXCoordinate, float initialYCoordinate, float rowLength, float columnLength)
        ...
        public bool IsEqual(Items.Item item)
        {
            bool result = false;
            if (item.GetSortingOrder() == _sortingGroup.sortingOrder)
            {
                result = true;
            }
            return result;
